...
public interface VendasItensViewRepository extends JpaRepository<VendasItensView, Integer> {
	...
	@Query(value = "SELECT\n" +
			...
			"AVG(( (select avg( case when ?3 = 0 then mve.precoultimacompra when ?3 = 1 then mve.custoaquisicao end  )  from movimentoestoque mve LEFT JOIN item  ON ( item.ID = mve.iditemoriginal )  LEFT JOIN operacao op ON ( op.ID = mve.idoriginal ) LEFT JOIN cfop  ON ( cfop.ID = item.idcfop )  where mve.data >= t1.emissao  and mve.data <= t1.emissao and mve.idproduto = t2.id AND mve.idfilial = filial.id and mve.tipodocumento IN ('1', '2') and mve.cancelado = '0' and mve.variacao='0' and mve.quantidadesaida > 0 and item.cancelado='0' and item.kit IN ( 0, 2 ) and op.tipo IN ( 1, 31 )  AND COALESCE ( op.statusnfce, '0' ) IN ( 0, 1, 13, 5, 6 )  and cfop.consideravenda='1'    )  )) AS preco_ultima_compra," +
			"AVG(( (select avg( case when ?3 = 0 then mve.precoultimacompra when ?3 = 1 then mve.custoaquisicao end )  from movimentoestoque mve  LEFT JOIN item  ON ( item.ID = mve.iditemoriginal )  LEFT JOIN operacao op ON ( op.ID = mve.idoriginal ) LEFT JOIN cfop  ON ( cfop.ID = item.idcfop )  where mve.data >= t1.emissao  and mve.data <= t1.emissao and mve.idproduto = t2.id AND mve.idfilial = filial.id and mve.tipodocumento IN ('1', '2') and mve.cancelado = '0'  and mve.variacao='0' and mve.quantidadesaida > 0 and item.cancelado='0' and item.kit IN ( 0, 2 ) and op.tipo IN ( 1, 31 ) AND COALESCE ( op.statusnfce, '0' ) IN ( 0, 1, 13, 5, 6 )  and cfop.consideravenda='1'   )   )) * SUM(t1.quantidade)   AS preco_ultima_compra_total," +
			...
			" GROUP BY  t1.produto,grupo_pai,grupo_filho,grupo_neto,descricao,codigo_filial,nome_filial, nome_promocao, promocao "
			 , nativeQuery = true)
	...
	@Query(value = "SELECT\n" +
			...
			"AVG(( (select avg( case when ?3 = 0 then mve.precoultimacompra when ?3 = 1 then mve.custoaquisicao end  )  from movimentoestoque mve LEFT JOIN item  ON ( item.ID = mve.iditemoriginal )  LEFT JOIN operacao op ON ( op.ID = mve.idoriginal ) LEFT JOIN cfop  ON ( cfop.ID = item.idcfop )  where mve.data >= t1.emissao  and mve.data <= t1.emissao and mve.idproduto = t2.id AND mve.idfilial = filial.id and mve.tipodocumento IN ('1') and mve.cancelado = '0' and mve.variacao='0' and mve.quantidadesaida > 0 and item.cancelado='0' and item.kit IN ( 0, 2 ) and op.tipo IN ( 1, 31 )  AND COALESCE ( op.statusnfce, '0' ) IN ( 0, 1, 13, 5, 6 )  and cfop.consideravenda='1'    )  )) AS preco_ultima_compra," +
			"AVG(( (select avg( case when ?3 = 0 then mve.precoultimacompra when ?3 = 1 then mve.custoaquisicao end )  from movimentoestoque mve  LEFT JOIN item  ON ( item.ID = mve.iditemoriginal )  LEFT JOIN operacao op ON ( op.ID = mve.idoriginal ) LEFT JOIN cfop  ON ( cfop.ID = item.idcfop )  where mve.data >= t1.emissao  and mve.data <= t1.emissao and mve.idproduto = t2.id AND mve.idfilial = filial.id and mve.tipodocumento IN ('1') and mve.cancelado = '0'  and mve.variacao='0' and mve.quantidadesaida > 0 and item.cancelado='0' and item.kit IN ( 0, 2 ) and op.tipo IN ( 1, 31 ) AND COALESCE ( op.statusnfce, '0' ) IN ( 0, 1, 13, 5, 6 )  and cfop.consideravenda='1'   )   )) * SUM(t1.quantidade)   AS preco_ultima_compra_total," +
