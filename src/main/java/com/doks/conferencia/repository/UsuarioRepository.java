...
public interface UsuarioRepository extends JpaRepository<Usuario, Integer> {
	...
	@Query(value = "select id,codigo,nome,senha,supervisor from usuario where codigo =? and senha =? " , nativeQuery = true)
	List<Usuario> findByCodigoAndsenha(String codigo, String senha);

	@Query(value = "select id, nome, senha, codigo,supervisor from usuario where codigo =?" , nativeQuery = true)
