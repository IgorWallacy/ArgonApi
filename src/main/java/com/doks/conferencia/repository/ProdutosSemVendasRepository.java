...
public interface ProdutosSemVendasRepository extends JpaRepository<ProdutosSemVendas, Integer> {
	...
	@Query(value=" select t1.id as id,"
			...
			+ " and ( select MAX(viv.emissao) from vendas_itens_view viv  where viv.produto = t1.codigo and viv.tipoitem='P' and viv.filial= CAST( ?3 AS TEXT) group by viv.emissao  order by viv.emissao desc limit 1) between ?5 and  cast(now() as date) "
			+ " and ( select quantidade from total_estoque_view ev  where ev.idproduto = t1.id and ev.idfilial = ?3) is not null ", nativeQuery = true)
	List<ProdutosSemVendas> todos(LocalDate data1, LocalDate data2, Integer filial, LocalDate ultimaCompra, LocalDate ultimaVenda);
	...
	@Query(value=" select t1.id as id,"
			...
			+ " and ( select MAX(viv.emissao) from vendas_itens_view viv  where viv.produto = t1.codigo and viv.tipoitem='P' and viv.filial= CAST( ?3 AS TEXT) group by viv.emissao  order by viv.emissao desc limit 1) between ?5 and  cast(now() as date) "
			+ " and ( select quantidade from total_estoque_view ev  where ev.idproduto = t1.id and ev.idfilial = ?3) is not null ", nativeQuery = true)
	List<ProdutosSemVendas> todos1Filial(LocalDate data1, LocalDate data2, Integer filial, LocalDate ultimaCompra, LocalDate dataUltimaVenda);
